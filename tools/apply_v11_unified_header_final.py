...
# --- 1) Replace shared shell CSS only ---
start = ui.index('<style data-weld-ui-shell-v3>')
end = ui.index('</style>', start) + len('</style>')
new_css = r'''<style data-weld-ui-shell-v3>
...
ui = ui[:start] + new_css + ui[end:]

# --- 2) Replace progress shell block only ---
progress_start = ui.index('        if progress:')
entry_start = ui.index('        elif entry:', progress_start)
progress_block = r'''        if progress:
...
"""
'''
ui = ui[:progress_start] + progress_block + ui[entry_start:]

# --- 3) Replace entry shell block only ---
entry_start = ui.index('        elif entry:')
thumb_start = ui.index('        elif thumbs:', entry_start)
entry_block = r'''        elif entry:
...
"""
'''
ui = ui[:entry_start] + entry_block + ui[thumb_start:]

# --- 4) Entry: current thumbnail is selected AND disabled, without changing load logic ---
old_css = '.thumb.active{border-color:#1967d2;background:#e8f0fe}'
new_css_entry = '.thumb.active{border-color:#1967d2;background:#e8f0fe}.thumb.active:disabled{opacity:1;border-color:#1967d2;background:#e8f0fe;color:#174ea6;cursor:default;box-shadow:inset 0 0 0 1px #1967d2}.thumb.active:disabled img{opacity:.86}'
assert entry.count(old_css) == 1, entry.count(old_css)
entry = entry.replace(old_css, new_css_entry, 1)
old_update = "function updateThumbActive(){thumbs.querySelectorAll('.thumb').forEach(b=>b.classList.toggle('active',Number(b.dataset.page)===Number(pageInput.value)));const n=Number(pageInput.value);ensureThumbLoaded(n);if(n>1)ensureThumbLoaded(n-1);if(n<pageCount)ensureThumbLoaded(n+1);const active=thumbs.querySelector('.thumb.active');if(active)active.scrollIntoView({block:'nearest',inline:'nearest'});}"
new_update = "function updateThumbActive(){const n=Number(pageInput.value);thumbs.querySelectorAll('.thumb').forEach(b=>{const active=Number(b.dataset.page)===n;b.classList.toggle('active',active);b.disabled=active;if(active){b.setAttribute('aria-current','page');b.title=`P${b.dataset.page}（表示中）`}else{b.removeAttribute('aria-current');b.title=`P${b.dataset.page}へ移動`}});ensureThumbLoaded(n);if(n>1)ensureThumbLoaded(n-1);if(n<pageCount)ensureThumbLoaded(n+1);const active=thumbs.querySelector('.thumb.active');if(active)active.scrollIntoView({block:'nearest',inline:'nearest'});}"
assert entry.count(old_update) == 1, entry.count(old_update)
entry = entry.replace(old_update, new_update, 1)

# Keep the original disabled behavior everywhere else; specifically override only navigation/current-page UI.
UI.write_text(ui, encoding='utf-8')
ENTRY.write_text(entry, encoding='utf-8')
print('Applied final unified header patch to ui_shell.py + project_entry.html')
