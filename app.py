...
def _dashboard_table_exists(connection, table_name):
...
@app.get("/progress-dashboard-data")
def progress_dashboard_data():
    if not DB_PATH.exists():
        response = jsonify(_dashboard_empty_payload())
    ...
    connection.row_factory = sqlite3.Row
    try:
        if not _dashboard_table_exists(connection, "projects"):
            response = jsonify(_dashboard_empty_payload())
            response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
            response.headers["Pragma"] = "no-cache"
            return response

        has_number_map = _dashboard_table_exists(connection, "number_map")
        has_weld_progress = _dashboard_table_exists(connection, "weld_progress")
    ...
    response = jsonify({"totals": totals, "projects": projects})
    response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    response.headers["Pragma"] = "no-cache"
    return response
