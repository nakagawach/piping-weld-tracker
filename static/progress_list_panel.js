(() => {
  ...
  function scrollSelected(){
    requestAnimationFrame(()=>records.querySelector('.progress-list-record.selected')?.scrollIntoView({block:'nearest',behavior:'auto'}));
  }
  function scrollCurrentPage(){
    requestAnimationFrame(()=>records.querySelector('.progress-list-record.current-page')?.scrollIntoView({block:'nearest',behavior:'auto'}));
  }
  function render(){
    ...
    records.innerHTML='';
    if(!filtered.length){
      records.innerHTML='<div class="progress-list-empty">条件に一致する項目がありません。</div>';
      return;
    }
    for(const item of filtered){
      const row=document.createElement('div');
      row.className='progress-list-record';
      ...
      focus.type='button';focus.className='progress-list-focus';
      ...
      focus.onclick=()=>{
        selectedKey=itemKey(item);render();scrollSelected();
        window.dispatchEvent(new CustomEvent('weld:progress-panel-target',{detail:{...item,openEditor:false}}));
      };

      const input=document.createElement('button');
      input.type='button';input.className='progress-list-input';input.textContent='進捗入力';
      input.onclick=event=>{
        event.stopPropagation();selectedKey=itemKey(item);render();scrollSelected();
        window.dispatchEvent(new CustomEvent('weld:progress-panel-target',{detail:{...item,openEditor:true}}));
      };
  ...
  async function loadList(){
    ...
    loading=true;state.textContent='読込中…';
    try{
      const response=await fetch(listUrl,{cache:'no-store'});
      const data=await response.json();
      if(!response.ok)throw new Error(data.error||'進捗一覧を取得できませんでした。');
      allItems=Array.isArray(data.items)?data.items:[];
      loaded=true;state.textContent=`${allItems.length}件`;render();scrollCurrentPage();
    }catch(error){
      state.textContent='取得失敗';records.innerHTML=`<div class="progress-list-empty">${error.message}</div>`;
  ...
  tabs.onclick=event=>{
    ...
    filter=button.dataset.filter;tabs.querySelectorAll('[data-filter]').forEach(item=>item.classList.toggle('active',item===button));render();
  };
  ...
  window.addEventListener('weld:progress-selection',event=>{
    ...
    selectedKey=`${Number(detail.pageNumber)||currentPage}:${Math.round(Number(detail.x))}:${Math.round(Number(detail.y))}`;
    currentPage=Number(detail.pageNumber)||currentPage;
    if(loaded){render();scrollSelected()}
  });
  ...
  window.addEventListener('weld:progress-saved',event=>{
    const saved=event.detail||{},key=itemKey(saved),index=allItems.findIndex(item=>itemKey(item)===key);
    if(index>=0)allItems[index]={...allItems[index],...saved};
    selectedKey=key;
    if(loaded){render();scrollSelected()}
  });
